...
class ksketchsvg:
    ...
    def get_spatial_keys(kobject):
        ...
        for spatial_key in kobject.findall('.//spatialkey'):
            ...
            keylist.append(d)
        #print "spatial_key: obj %d, time %.1f" % (int(kobject.attrib['id']), time)
        return keylist
    ...
    def get_visibility_keys(kobject):
        ...
        for visibility_key in kobject.findall('.//visibilitykey'):   #List of ElementTree in document order
            ...
            d['v'] = bool(visibility_key.attrib['visibility'])
            # print "visibility_key: obj %d, time %f, visible %d" % (int(kobject.attrib['id']), d['time'], d['v'])
            keylist.append(d)
        return keylist
    ...
    def get_object_frame(time, skeys, vkeys, mem):
        ...
        if time == vkeys[mem['vkey_idx']]['time']:
    ...
    def get_transformations2(xml):
        ...
        timeline = {}
        for id in timelines:
            for time in timelines[id]:
                if time in timeline:
                    arr = timeline[time]
                else:
                    arr = []
                    timeline[time] = arr
                arr.append(timelines[id][time])

        return collections.OrderedDict({ 'centers': centers, 'timeline':timeline })
    ...
    def get_transformations(xml):
        ...
        for kobject in root.findall('.//KObject'):
            ...
            objectID = kobject.attrib['id']
            for visibilityKey in kobject.findall('.//visibilitykey'):
                time_v = float(visibilityKey.attrib['time'])
                visibility = "1"
                if visibilityKey.attrib['visibility'] == "false":
                    visibility = "0"
                    ...
                    if path_type == 'translate':
                        ...
                        if points != '':
                            for point in points.split():
                                ...
                                if time in time_frame:
                                    arr = time_frame[time]
                                    arr[1] = [x - prev_angle, c_off_x, c_off_y]
                                else:
                                    arr = [[], [], []]
                                    arr[1] = [x - prev_angle, c_off_x, c_off_y]
                                    time_frame[time] = arr
        ...
        for key in visibility_frames:
            for time in visibility_frames[key]:
                if time in time_line:
                    time_line[time].append({"obj": key, "v": visibility_frames[key][time]})
                else:
                    time_line[time] = [{"obj": key, "v": visibility_frames[key][time]}]
